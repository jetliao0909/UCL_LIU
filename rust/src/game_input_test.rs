...
mod tests {
    ...
    #[test]
    fn test_game_input_tester_creation() {
        let _tester = GameInputTester::new();
        // 測試創建是否成功
    }
    ...
    fn test_key_click() {
        ...
        // 注意：這個測試會實際發送按鍵，可能會影響系統
        // 在 CI 環境中可能需要跳過
        // tester.key_click(Key::Space);
    }
    ...
    fn test_key_combination() {
        let mut tester = GameInputTester::new();
        // 注意：這個測試會實際發送按鍵，可能會影響系統
        // tester.key_combination(&[Key::Control, Key::Space]);
    }

    #[test]
    fn test_char_to_key() {
        assert_eq!(char_to_key('a'), Some(Key::Layout('a')));
        assert_eq!(char_to_key('A'), Some(Key::Layout('a')));
        assert_eq!(char_to_key('1'), Some(Key::Layout('1')));
        assert_eq!(char_to_key(' '), Some(Key::Space));
        assert_eq!(char_to_key('中'), None); // 中文字符無法轉換
    }
}
