...
mod tests {
    ...
    fn test_shift_toggle_state() {
        // 測試 Shift 切換狀態（僅測試狀態本身，不模擬實際鍵盤事件）
        // 實際行為：
        // - 單獨按一下 Shift（期間沒有搭配其他鍵）→ 在放開時切換「攔截 / 英模式」，並清除現有字根
        // - Shift + 其他鍵（例如 Shift+1, Shift+A）→ 視為一般組合鍵，不切換模式
        // - 即使在英模式（不攔截）下，只要 Shift 期間有搭配其他鍵，放開 Shift 也不會切換模式
        SHIFT_TOGGLE.with(|t| {
            // 初始狀態應該是 false（攔截模式）
            *t.borrow_mut() = false;
            assert!(!*t.borrow());
